...
func (c *ConsumeClient) Subscriber(ctx context.Context, address string, chb *ChannelBindings, opb *OperationBindings, security run.AnySecurityScheme) (Subscriber, error) {
	sec := c.security
	if security != nil {
		sec = security
	}
...
func (c *ConsumeClient) ensureChannel(channelName string, chb *ChannelBindings, opb *OperationBindings, sec run.AnySecurityScheme) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *ConsumeClient) checkSecurity(req *http.Request, sec run.AnySecurityScheme) (bool, error) {
